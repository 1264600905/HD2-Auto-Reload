...
- 将原生换弹从 GL-15 空仓原型扩展为按当前武器的 Reload 组件、有效配置及能力状态选择原生调用；无可验证组件时回退模拟 R。
- 恢复现有战术换弹配置，GL-15 在弹匣加膛内总弹 ≤2 时进入换弹窗口；逐发续装须等待弹药进展与原生动作空闲，避免重入。
- 沿用 build 25480438 的原生指令、身份与构建 SHA256 校验。仅 GL-15 空仓原型已有用户实机成功反馈，新覆盖仍待实测。
- 发布普通、战术、原生及原生战术四种独立安装包，并保留对应诊断包；运行时版本与包名统一为 0.7.0。

## 0.6.3

- 修复独立加载时 SendInput 缺少 FFI 声明导致初始化失败，保留已有声明的兼容处理。
- 增加 Windows 独立进程初始化测试，覆盖无声明及已有不同指针声明两种情况，不发送按键。
- 修正输入测试桩的 void 指针读取，51 项行为测试全部通过。

## 0.6.2

- 支持 Steam build 25480438，保留 25327279 兼容性。
- 实机只读核对关键指令及三类完整组件表，保持原有布局、身份和状态校验。
- 统一实机诊断脚本和 Mod 的版本校验，构建校验要求 DLL/EXE 属于同一受支持版本。
